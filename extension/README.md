...
**バージョン 0.0.0.2** ・ 移植元: pyside6-webusb v0.0.4b1
...
2. [アーキテクチャ](#アーキテクチャ)
3. [v0.0.0a1で追加された機能](#v000a1で追加された機能)
4. [インストール](#インストール)
5. [セキュリティモデル](#セキュリティモデル)
6. [pyside6-webusb からの移植で変えたところ](#pyside6-webusb-からの移植で変えたところ)
7. [0.0.0a0: 実機検証(Windows/LibreWolf)で見つかった不具合と修正](#000a0-実機検証windowslibrewolfで見つかった不具合と修正)
8. [既知の限界・未検証事項](#既知の限界未検証事項)
9. [ディレクトリ構成](#ディレクトリ構成)
10. [テスト](#テスト)
11. [ライセンス](#ライセンス)
...
┌─────────────────────────── Firefox ───────────────────────────────────┐
│                                                                          │
│  Webページ (例: https://example.com)         拡張機能自身のページ         │
│  ┌──────────────────────────────┐        ┌───────────────────────┐   │
│  │ page_polyfill.js (MAIN world)  │        │ popup.js               │   │
│  │  ├ webusb_core.js に依存         │        │  navigator.usb相当を   │   │
│  │  navigator.usb.requestDevice() │        │  background page越しに │   │
│  │  / transferIn() 等              │        │  直接利用              │   │
│  │        │ window.postMessage()  │        └───────────┬───────────┘   │
│  └────────┼───────────────────────┘                    │ getBackgroundPage()
│           │                                             ▼               │
...
│  │   ・webusb_core.js を直接(postMessage不要で)駆動する               │   │
│  │     "Firefox-style surface" 自身の navigator.usb も、ここで持つ     │   │
│  └────────┼───────────────────────────────────────────────────────┘   │
└───────────┼─────────────────────────────────────────────────────────┘
...
USBデバイスへの実際のアクセス経路(ネイティブホストへの唯一の窓口である
`background.js`の`dispatch`/`callNative`、その先の`bridge.py`/`hardening.py`が
実装するWebUSBセキュリティモデルそのもの)は、常に1つだけです。その上に、
2つの異なる「顔」を用意しています:

```
        USB backend (pyusb/libusb)
             │
             ▼
        USB compatibility core (bridge.py / hardening.py)
             │
   ┌─────────┴─────────┐
   ▼                     ▼
...
- **Chromium surface** — `content_script.js`が任意のWebページのMAIN world
  へ注入する`page_polyfill.js`。`window.postMessage()`を経由し、実際の
  Chromeが持つ`navigator.usb`と見分けが付かないことを目指した、ページ向けの顔。
- **Firefox-style surface** — 拡張機能自身の特権的なページ
  (`background.js`自身、および`browser.runtime.getBackgroundPage()`経由の
  `popup.js`)が、`postMessage`/`content_script.js`の中継を一切経由せず、
  `background.js`の`dispatch`関数を直接呼び出して使う、拡張機能内部向けの顔。
  これはWICGが実際に提案している
  ...
  実際の仕様の方向性——と同じ発想を、Manifest V2の永続的background pageという
  fox-webusb自身のアーキテクチャに合わせて実装したものです。

両サーフェスの実体(クラス定義・エラー変換・base64変換・イベントの形)は
`extension/webusb_core.js`という単一の共有ファイルにあります。「Firefox風」
なのはあくまで配線(トランスポート)の話であって、WebUSB仕様が定める
振る舞い自体は両サーフェスで一切変わりません——実装が2つあることで
挙動が食い違う、という事態がそもそも構造的に起こり得ない設計です。

`requestDevice()`のユーザー操作要件だけは、サーフェスごとに意味が変わります:
`page_polyfill.js`・`popup.js`(実際にクリックを受け取れる、本物のページ)
では仕様どおり検証されますが、`background.js`自身(常駐する、通常は
ユーザー操作を一切受け取らないページ)から直接`requestDevice()`を呼んでも
まず成功しません——実際のChrome拡張機能向けドキュメントが案内している
パターンと同じく、`popup.js`側で`requestDevice()`を呼んでいったん許可を
取り、`background.js`側は`getDevices()`で既に許可済みのデバイスを拾って
実際の通信を続ける、という使い方を想定しています。

## v0.0.0a1で追加された機能

詳しい経緯・設計判断は[CHANGELOG.md](CHANGELOG.md) [0.0.0a1]を参照。ここでは
何ができるようになったかだけを簡潔にまとめる。

- **セキュリティ強化**: 独立したセキュリティ監査(pyside6-webusb側)で見つかった
  問題(alternate setting混同・ユーザー操作検証・デバイス文字列サニタイズ・
  ハンドル数上限)を確認し、実際に該当したものを修正。
- **Permissions Policy対応**: `<iframe allow="usb">`で許可されていない
  クロスオリジンiframeには`navigator.usb`自体を公開しない(実仕様どおり)。
- **ローカルアテステーション**: `window.__foxWebUSB.extensions.attestation`
  — オリジンごとのEd25519鍵で、サイトが「同じローカルブリッジからの応答か」
  を検証できる、実ChromeのWebUSBには無い機能。
- **F12デバッグヘルパー**: `window.__foxWebUSB.bridgeInfo()`でバージョン
  確認、`.listGrantedDevices()`で許可済みデバイス一覧、
  `.explainTransferLimits()`で転送サイズ方針を確認できる。
- **UIの多言語対応**: popup/optionsページ、およびTkinterのデバイス選択
  ダイアログが日本語・英語・簡体字中国語(zh_CN)に対応(Firefox本体のUI言語に
  追従)。
- **`delete navigator.usb`への耐性**: 一度設置されると`delete`では取り除けない
  (ただし公式実装や他のWebUSB実装が既にあれば、そちらへ道を譲る)。
- **TypeScript型定義の拡充**: `types/fox-webusb-extensions.d.ts`で上記の
  独自機能群にも型が付くようになった。

`usb-unrestricted`(Isolated Web Appsが保護対象クラス・ブロックリストを
迂回できるようにする、Chrome/ChromeOS固有の新機能)は意図的に実装していない
——Firefox拡張機能にはこれを安全に成立させる検証基盤が無く、実装すれば
セキュリティキーやキーボードへの無制限アクセス経路を作るだけになるため。
...
- **Firefox for Androidでは動作しません。** `manifest.json`は
  `gecko_android`向けの互換バージョンを宣言していますが(v0.0.0a1で追加した
  `data_collection_permissions`フィールドの要件を満たすため)、これは
  「インストールできる」という意味でしかありません。fox-webusbの根幹である
  `nativeMessaging`権限自体がFirefox for Androidでサポートされていない
  (複数のMozilla公式フォーラムでの言及・実装状況で確認済み)ため、Android上
  では常に「ネイティブホストが見つからない」状態になります——クラッシュは
  しませんが(background.js自身が"host not available"を検知して素直に
  失敗する既存の設計のおかげ)、実質的に何もできません。
...
# page_polyfill.js (window/navigator/postMessageをNodeでモックした機能テスト、44件)
node tests/test_page_polyfill.js

# background.js の Firefox-style surface (最小限のbrowser.*モック上での検証、6件)
node tests/test_extension_surface.js

# Rustアクセラレーション(素のRustロジック層、13件)
cd native/fox_webusb_accel && cargo test --release

# 型定義 (.d.ts が正しい使い方を受理し、誤った使い方を型エラーとして拒否することの確認)
cd types && npx tsc --noEmit --strict --lib es2020,dom \
  webusb-polyfill.d.ts sample-usage.ts negative-check.ts \
  fox-webusb-extensions.d.ts fox-webusb-extensions-sample-usage.ts fox-webusb-extensions-negative-check.ts
```

いずれも実USBデバイス・実Firefoxを必要としない、ロジック単体の検証です
(前述「既知の限界」参照)。i18n(`_locales/*/messages.json`・`i18n.py`)の
実際の表示崩れ・訳文の自然さそのものは、この種のロジックテストでは検証
できない領域であり、実機での見た目確認が別途望ましい。
